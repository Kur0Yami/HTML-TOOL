...
class SaveFile{
	...
	manageCharms(slot){
		...
		var CHARM_BASE = 0x62EE;
		var CHARM_STRIDE = 36;
		var CHARM_MAX = 2000;

		var charmIndices = [];
		for(var i = 0; i < CHARM_MAX; i++){
			var offset = CHARM_STRIDE * i + CHARM_BASE;
			if(data[offset] == 0x6) charmIndices.push(i);
		}

		function skillOptions(selected){
			var opts = `<option value=0 ${selected == 0 ? "selected" : ""}>-- None --</option>`;
			for(var s = 0; s < skill_names.length; s++){
				opts += `<option value=${s+1} ${selected == s+1 ? "selected" : ""}>${skill_names[s]}</option>`;
			}
			return opts;
		}

		function rowHTML(index, count, skill1, pts1, skill2, pts2){
			return `<tr data-index="${index}">
				<td style="border: 1px solid grey; padding: 2px"><input type="number" class="charm-count" value="${count}" min="0" max="255" style="width: 50px"></td>
				<td style="border: 1px solid grey; padding: 2px"><select class="charm-skill1">${skillOptions(skill1)}</select></td>
				<td style="border: 1px solid grey; padding: 2px"><input type="number" class="charm-pts1" value="${pts1}" min="0" max="255" style="width: 50px"></td>
				<td style="border: 1px solid grey; padding: 2px"><select class="charm-skill2">${skillOptions(skill2)}</select></td>
				<td style="border: 1px solid grey; padding: 2px"><input type="number" class="charm-pts2" value="${pts2}" min="-127" max="127" style="width: 50px"></td>
				<td style="border: 1px solid grey; padding: 2px"><button class="charm-delete">Delete</button></td>
			</tr>`;
		}

		var info = `<div class="cat-window"><span id="charmcount">${charmIndices.length} charm(s) found</span></br></br>
			<div style="max-height: 400px; overflow-y: auto">
			<table id="charm-table" style="width: 100%; border-collapse: collapse; text-align: center">
			<tr>
				<th style="border: 1px solid grey">Slots</th><th style="border: 1px solid grey">Skill 1</th><th style="border: 1px solid grey">Pts 1</th><th style="border: 1px solid grey">Skill 2</th><th style="border: 1px solid grey">Pts 2</th><th style="border: 1px solid grey"></th>
			</tr>`;

		charmIndices.forEach(i => {
			var offset = CHARM_STRIDE * i + CHARM_BASE;
			var count = data[offset+16];
			var skill1 = data[offset+12];
			var pts1 = data[offset+14];
			var skill2 = data[offset+13];
			var pts2raw = data[offset+15];
			var pts2 = pts2raw > 128 ? pts2raw - 256 : pts2raw;
			info += rowHTML(i, count, skill1, pts1, skill2, pts2);
		});

		info += `</table></div>
			<span style="display: block; margin-top: 10px">
				<button id="charm-add" style="margin-right: 10px">Add New Charm</button>
				<button id="charm-save">Save</button>
			</span>
			<p style="font-size: 11px; color: grey; max-width: 480px">
				Only the Skill/Points/Slots fields (the ones this tool already knew how to export) are edited here.
				Deleting a charm just clears its marker byte. Newly added charms use zeroed unknown bytes and are
				experimental &mdash; back up your save before using this, and test in-game.
			</p>
		</div>`;

		popup.innerHTML = info;

		var table = document.getElementById("charm-table");

		function attachDelete(row){
			row.querySelector(".charm-delete").addEventListener("click", (e) => {
				e.target.closest("tr").remove();
			});
		}

		table.querySelectorAll("tr[data-index]").forEach(attachDelete);

		document.getElementById("charm-add").addEventListener("click", () => {
			var used = new Set();
			table.querySelectorAll("tr[data-index]").forEach(r => used.add(parseInt(r.dataset.index)));
			var freeIndex = -1;
			for(var i = 0; i < CHARM_MAX; i++){
				if(!used.has(i)){ freeIndex = i; break; }
			}
			if(freeIndex == -1){
				alert("No free charm slots available.");
				return;
			}
			var tmp = document.createElement("tbody");
			tmp.innerHTML = rowHTML(freeIndex, 1, 0, 0, 0, 0);
			var row = tmp.firstElementChild;
			table.appendChild(row);
			attachDelete(row);
		});

		document.getElementById("charm-save").addEventListener("click", () => {
			// Clear the marker byte on every originally-found charm slot first,
			// so deleted rows actually disappear.
			charmIndices.forEach(i => {
				var offset = CHARM_STRIDE * i + CHARM_BASE;
				data[offset] = 0;
			});
			...
			table.querySelectorAll("tr[data-index]").forEach(row => {
				...
				var offset = CHARM_STRIDE * i + CHARM_BASE;
				var count = parseInt(row.querySelector(".charm-count").value) || 0;
				var skill1 = parseInt(row.querySelector(".charm-skill1").value) || 0;
				var pts1 = parseInt(row.querySelector(".charm-pts1").value) || 0;
				var skill2 = parseInt(row.querySelector(".charm-skill2").value) || 0;
				var pts2 = parseInt(row.querySelector(".charm-pts2").value) || 0;
				if(pts2 < 0) pts2 += 256;

				data[offset] = 0x6;
				data[offset+12] = skill1 & 0xFF;
				data[offset+13] = skill2 & 0xFF;
				data[offset+14] = pts1 & 0xFF;
				data[offset+15] = pts2 & 0xFF;
				data[offset+16] = count & 0xFF;
			});
    ...
    save.save_slots.forEach((slot, index) => {
        ...
        if (slot.name) {
		  ...
		  <div class="slot-actions">
			...
			<button onclick="editCharms(${index})">Edit Charms</button>
		  </div>`;
        } else {
...
function editCharms(slot){
	save.manageCharms(slot);
}
